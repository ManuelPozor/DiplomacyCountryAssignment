...
for player in players:
    ...
    unwanted_countries=[(country, 5) for country in unique_country_tags if country not in [player["prio1"],player["prio2"],player["prio3"]]]
...
for comb in combinations:
    ...
    if len(set(selected_countries))<len(selected_countries): # if set smaller, duplicates exist
        continue
...
with open("country_distribution.txt", "w") as file:
    for ind, country_weight_tuple in enumerate(best_combination):
        ...
        file.write(f"Player {ind+1}: {player_name} {country_weight_tuple[0]}\n")
